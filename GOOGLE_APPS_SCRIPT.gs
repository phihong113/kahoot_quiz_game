 ...
 * (BẢO MẬT CHỐNG CRASH KHI CÓ TRANG BẢO VỆ & HOÀN TOÀN TỰ ĐỘNG)
 * ============================================================================
...
function doGet(e) {
  try {
    ...
    // TỰ ĐỘNG KHỞI TẠO HÀNG TIÊU ĐỀ NẾU CÓ THỂ WRAP AN TOÀN
    try {
    ...
    if (action === 'google_auth' || action === 'check_teacher') {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (rowEmail === email) {
          ...
          // TỰ ĐỘNG CẬP NHẬT CỘT LASTLOGIN AN TOÀN
          try {
      ...
      if (foundRowIndex === -1) {
        try {
          ...
          // Bỏ qua lỗi nếu sheet bị khóa trang bảo vệ
        }
    ...
    return jsonResponse({ success: false, message: 'Lỗi xử lý hệ thống: ' + globalErr.toString() });
  }
}
